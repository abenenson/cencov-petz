...
@[simp] lemma split_pushforward_apply (p : Simplex α) (a : α) (i : Fin (m a)) :
    ((split (α := α) m hm).pushforward p).p ⟨a, i⟩ = p.p a / (m a : ℝ) := by
  classical
  -- Expand the definition and evaluate the `if`-sum.
  simp [MarkovMorphism.pushforward_apply, split, mul_ite, div_eq_mul_inv, eq_comm]

@[simp] lemma split_tangentPushforward_apply (u : tangentSpace (α := α)) (a : α) (i : Fin (m a)) :
